    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            if (r.ok && data && data.ok) {
                ...
                addLog("Refresh the page to see the track in your library.", "info");
                setStatus("done", "OK");
            } else {
                addLog(`ERROR: ${text || "unknown"}`, "err");
                setStatus("err", "FAIL");
            }
        } catch (err) {
            addLog(`COMMUNICATION ERROR: ${err}`, "err");
            setStatus("err", "FAIL");
        } finally {
            btn.disabled = false;
            input.value = "";
...
function showZipReady(zipUrl, autoDownload) {
    ...
    addLog(`Download ZIP: ${zipUrl}`, "ok");
    if (autoDownload) {
        addLog("Downloading automatically...", "info");
...
async function pollSpotifyJob(jobId) {
    ...
    while (true) {
        ...
        try {
            ...
            if (!r.ok) {
                addLog(`HTTP ERROR ${r.status} while polling status`, "err");
                setStatus("err", "FAIL");
                return;
            }
            data = await r.json();
        ...
        if (!data || !data.ok) {
            addLog(`ERROR: ${(data && data.error) || "unknown"}`, "err");
            setStatus("err", "FAIL");
            return;
        }
        ...
        if (data.state === "done") {
            if (data.zip_url) {
                ...
                addLog("Job finished but no ZIP was produced.", "err");
            }
            return;
        }
...
function initSpotifyForm() {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        addLog(`> limit=${limit}, save to library=${save === "1" ? "yes" : "no"}`, "info");
        addLog("Starting background job...", "info");
        ...
        try {
            ...
            if (!r.ok) {
                ...
                addLog(`HTTP ERROR ${r.status}: ${t || r.statusText}`, "err");
                setStatus("err", "FAIL");
                return;
            }
            const data = await r.json();
            if (!data || !data.ok || !data.job_id) {
                addLog(`ERROR: ${(data && data.error) || "failed to start"}`, "err");
                setStatus("err", "FAIL");
                return;
            }
            jobId = data.job_id;
    ...
    function apply() {
        ...
        if (counter) {
            counter.textContent = q
                ? `${shown} of ${total} match "${input.value.trim()}"`
                : `Total tracks: ${total}`;
        }
